...
func _build_static_table() -> void:
	...
	var back := _make_button("←", Vector2(20, 14), Vector2(72, 56), 34)
	back.pressed.connect(_on_back_pressed)
	var title := _make_label("Rami", Vector2(118, 4), Vector2(240, 72), 52, GOLD)
	...
	meld_layer.position = Vector2(425, 278)
	meld_layer.size = Vector2(710, 278)
...
func _build_opponent_header() -> void:
	var p := _panel(Vector2(630, 8), Vector2(340, 72), Color("#0E2B28"), Color("#456F69"), 26, 2)
	var icon := _panel(Vector2(12, 8), Vector2(56, 56), Color("#14262A"), Color("#7D9895"), 28, 2, p)
	_make_label("IA", Vector2(0, 4), Vector2(56, 48), 22, TEXT, HORIZONTAL_ALIGNMENT_CENTER, icon)
	_make_label("Adversaire", Vector2(84, 5), Vector2(180, 28), 22, TEXT, HORIZONTAL_ALIGNMENT_LEFT, p)
	opponent_count_label = _make_label("", Vector2(84, 34), Vector2(120, 25), 18, GOLD, HORIZONTAL_ALIGNMENT_LEFT, p)
	ai_open_label = _make_label("", Vector2(205, 34), Vector2(120, 25), 16, MUTED, HORIZONTAL_ALIGNMENT_RIGHT, p)

func _build_score_panel() -> void:
	var p := _panel(Vector2(1324, 12), Vector2(246, 166), Color("#10151C"), Color("#41505F"), 18, 2)
	_make_label("PARTIE SOLO", Vector2(16, 8), Vector2(214, 30), 22, TEXT, HORIZONTAL_ALIGNMENT_CENTER, p)
	...
	p.add_child(sep)
	_make_label("Vous", Vector2(18, 87), Vector2(76, 28), 20, TEXT, HORIZONTAL_ALIGNMENT_LEFT, p)
	player_open_label = _make_label("Non ouvert", Vector2(96, 87), Vector2(130, 28), 17, MUTED, HORIZONTAL_ALIGNMENT_RIGHT, p)
	_make_label("IA", Vector2(18, 123), Vector2(76, 28), 20, TEXT, HORIZONTAL_ALIGNMENT_LEFT, p)
	_make_label("51 + tierce franche", Vector2(96, 123), Vector2(130, 28), 14, MUTED, HORIZONTAL_ALIGNMENT_RIGHT, p)
...
func _build_center_zone() -> void:
	...
	_make_label("TABLE", Vector2(690, 283), Vector2(180, 28), 18, Color(0.75, 0.9, 0.82, 0.55), HORIZONTAL_ALIGNMENT_CENTER)
...
func _build_piles() -> void:
	...
	var stock_button := TextureButton.new()
...
func _build_turn_panel() -> void:
	var p := _panel(Vector2(1175, 345), Vector2(338, 104), Color("#0C171B"), Color("#1A443A"), 20, 2)
	_make_label("▶", Vector2(18, 20), Vector2(50, 48), 32, GREEN, HORIZONTAL_ALIGNMENT_CENTER, p)
	turn_label = _make_label("", Vector2(70, 14), Vector2(250, 76), 20, TEXT, HORIZONTAL_ALIGNMENT_CENTER, p)

func _build_bottom_bar() -> void:
	var sort_button := _make_button("▣  Trier", Vector2(28, 820), Vector2(185, 64), 24)
	sort_button.pressed.connect(_on_sort_pressed)
	var clear_button := _make_button("Annuler sélection", Vector2(225, 820), Vector2(225, 64), 21)
	clear_button.pressed.connect(_on_clear_selection)

	var player := _panel(Vector2(610, 814), Vector2(350, 74), Color("#101A20"), Color("#7C8E9A"), 28, 2)
	...
	hand_count_label = _make_label("", Vector2(88, 37), Vector2(210, 26), 18, GOLD, HORIZONTAL_ALIGNMENT_LEFT, player)
	...
	end_button = _make_button("Défausser • Fin du tour", Vector2(1205, 820), Vector2(365, 64), 22, Color("#0B5E40"), GREEN)
	end_button.pressed.connect(_on_end_turn)
	_make_label("v0.0.4 PLAYABLE", Vector2(8, 884), Vector2(160, 16), 13, Color(1, 1, 1, 0.42))
...
func _refresh_all() -> void:
	...
	if game.winner != "":
		...
		end_button.disabled = selected_indices.size() != 1

func _refresh_opponent_hand() -> void:
	_clear_children(opponent_layer)
	var count := game.ai_hand.size()
	var overlap := 43.0 if count <= 16 else 600.0 / max(1.0, float(count - 1))
	var total := 60.0 + overlap * float(max(0, count - 1))
	var start_x := 800.0 - total / 2.0
	for i in range(count):
		var y := 112.0 + abs(float(i) - float(count - 1) / 2.0) * 1.05
		_make_card_widget("back_red", Vector2(start_x + float(i) * overlap, y), Vector2(60, 84), (float(i) - float(count - 1) / 2.0) * 0.35, false, -1, opponent_layer, false)

func _refresh_player_hand() -> void:
	_clear_children(player_layer)
	var count := game.player_hand.size()
	if count == 0:
		return
	var card_w := 112.0
	var card_h := card_w / CARD_RATIO
	var overlap := min(78.0, 1080.0 / max(1.0, float(count - 1)))
	var total_width := card_w + overlap * float(count - 1)
	var start_x := 800.0 - total_width / 2.0
	for i in range(count):
		var center_delta := float(i) - float(count - 1) / 2.0
		var y := 615.0 + abs(center_delta) * 0.9
		var selected := selected_indices.has(i)
		if selected:
			y -= 28.0
		_make_card_widget(game.player_hand[i], Vector2(start_x + float(i) * overlap, y), Vector2(card_w, card_h), center_delta * 0.16, game.phase == "discard", i, player_layer, selected)

func _refresh_discard() -> void:
	_clear_children(discard_layer)
	var top := game.top_discard()
	if top != "":
		_make_card_widget(top, Vector2(12, 0), Vector2(118, 166), 0.0, game.phase == "draw", -1, discard_layer, false)
...
func _refresh_melds() -> void:
	...
	if game.table_melds.is_empty():
		_make_label("Aucune combinaison posée", Vector2(155, 105), Vector2(400, 40), 22, Color(0.75, 0.88, 0.80, 0.45), HORIZONTAL_ALIGNMENT_CENTER, meld_layer)
		return
	var x := 15.0
	var y := 38.0
	for meld in game.table_melds:
		...
		var w := 58.0
		var h := 82.0
		var meld_width := 22.0 * float(max(0, cards.size() - 1)) + w
		if x + meld_width > 690.0:
			x = 15.0
			y += 112.0
		if y > 205.0:
			break
		var owner_color := GREEN if String(meld.owner) == "player" else RED
		var tag := _make_label("VOUS" if String(meld.owner) == "player" else "IA", Vector2(x, y - 23), Vector2(70, 20), 12, owner_color, HORIZONTAL_ALIGNMENT_LEFT, meld_layer)
		tag.mouse_filter = Control.MOUSE_FILTER_IGNORE
		for i in range(cards.size()):
			_make_card_widget(String(cards[i]), Vector2(x + float(i) * 22.0, y), Vector2(w, h), 0.0, false, -1, meld_layer, false)
		x += meld_width + 24.0
...
func _make_card_widget(card_id: String, pos: Vector2, card_size: Vector2, rotation: float, clickable: bool, index: int, parent: Control, selected: bool) -> Control:
	...
	st.border_color = GOLD if selected else Color(0, 0, 0, 0.52)
...
func _load_card_texture(card_id: String) -> Texture2D:
	var tex := load("res://assets/cards/%s.png" % card_id)
...
func _on_pose_pressed() -> void:
	if selected_indices.size() < 3:
		return
	var result := game.play_player_selection(selected_indices)
	if bool(result.ok):
		selected_indices.clear()
	else:
		game.last_message = String(result.message)
	_refresh_all()
...
func _show_game_over() -> void:
	...
	var panel := _panel(Vector2(480, 270), Vector2(640, 350), Color("#101A20"), GOLD if game.winner == "player" else RED, 28, 4, modal_layer)
	var heading := "VICTOIRE !" if game.winner == "player" else "DÉFAITE"
	_make_label(heading, Vector2(40, 35), Vector2(560, 80), 54, GOLD if game.winner == "player" else RED, HORIZONTAL_ALIGNMENT_CENTER, panel)
	_make_label("Vous avez vidé votre main." if game.winner == "player" else "L'IA a vidé sa main.", Vector2(45, 125), Vector2(550, 48), 24, TEXT, HORIZONTAL_ALIGNMENT_CENTER, panel)
	var replay := _make_button_in("REJOUER", Vector2(125, 220), Vector2(390, 76), 28, Color("#0B5E40"), GREEN, panel)
